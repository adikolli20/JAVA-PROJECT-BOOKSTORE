...
public static Parent newOrder(){
    ...
    tableView.getColumns().addAll(isbnCol, titleCol, purchasedPriceCol, sellingPriceCol, authorCol,publisherCol,dateCol,qcol);
    ...
    final TableColumn<Book, String> publisherCol_ = new TableColumn<>("Publisher");
    ...
    publisherCol_.setCellValueFactory(
            new PropertyValueFactory<>("stock")
    );
    publisherCol_.setCellFactory(TextFieldTableCell.forTableColumn());
    ...
    tableView_.getColumns().addAll(isbnCol_, titleCol_, purchasedPriceCol_, sellingPriceCol_, authorCol_, publisherCol_,dateCol_,qcol_);
...
public static void saveToFile(){
    ...
    public static VBox printBill(BookOrder bookOrder){
        ...
        try {
            ...
            printWriter.println("Total Amount to be paied "+amount);
        ...
        billdisplay.getChildren().add(new HBox(new Label("Total Amount to be paied "+amount)));
...
public static boolean findTitle(ArrayList<Ordered_Book> books,String booktitle){
    ...
    public static void billstats(){
        ...
        for(BookOrder order: getOrders()){
            vb.getChildren().add(new Label("Amount "+order.getUser().getUsername()+order.getBooksOrdered().get(0).getTotamount()));
        }
